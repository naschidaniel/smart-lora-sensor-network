...
fn main() -> ! {
    ...
    let mut moisture_2_msg: String<20> = String::new();
    let mut moisture_3_msg: String<20> = String::new();
    ...
    loop {
        ...
        moisture_2_msg.clear();
        moisture_3_msg.clear();
        write!(moisture_1_msg, "SOIL 1: {moisture1:.0}%").unwrap();
        write!(moisture_2_msg, "SOIL 2: {moisture1:.0}%").unwrap();
        write!(moisture_3_msg, "SOIL 3: {moisture1:.0}%").unwrap();
        write!(
            lora_msg,
            "SENSOR_MOISTURE\tADC1: {adc_pin_0_counts}\t{moisture_1_msg}\tADC2: {adc_pin_0_counts}\t{moisture_2_msg}\tADC3: {adc_pin_0_counts}\t{moisture_3_msg}\tCOMPLETE_MOISTURE\n"
        )
        .unwrap();
        led_pin.set_high().unwrap();

        Text::new(&moisture_1_msg, Point::new(05, 10), text_style)
            .draw(&mut display)
            .unwrap();
        Text::new(&moisture_2_msg, Point::new(05, 35), text_style)
            .draw(&mut display)
            .unwrap();
        Text::new(&moisture_3_msg, Point::new(05, 60), text_style)
            .draw(&mut display)
            .unwrap();
